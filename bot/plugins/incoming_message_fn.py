...
async def incoming_compress_message_f(bot, update):
    ...
    try:
      if int(update.command[1]) <= 90 and int(update.command[1]) >= 10:
        ...
        try:
          await bot.send_message(
            chat_id=update.chat.id,
            text="The Value Should Be Under 10 - 90",
            reply_to_message_id=update.message_id
          )
          return
        except:
  ...
  if os.path.exists(saved_file_path):
    ...
    if duration is None or bitrate is None:
      try:
        await sent_message.edit_text(                
          text="Getting Media Of Meta Data Failed"                
        )
      except:
          pass          
      delete_downloads()
...
async def incoming_cancel_message_f(bot, update):
  ...
  if os.path.exists(status):
    ...
    await update.reply_text("It Will Stop The Compression", reply_markup=reply_markup, quote=True)
    delete_downloads()
    await bot.send_message(
      chat_id=update.chat.id,
